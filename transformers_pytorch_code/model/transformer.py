...
class MultiHeadAttention(nn.Module):
    def __init__(self, hidden_size, dropout_rate, head_size=8):
        ...
        self.attn_size = att_size = hidden_size // head_size
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, mask):
        y = self.self_atttention_norm(x)
        y = self.self_attention(y, y,y, mask)
        y = self.self_atttention_dropout(y)
        x = x + y
        return x
...
class Encoder(nn.Module):
    def __init__(self, hidden_size, dropout_rate, n_layers):
...
class Decoder(nn.Module):
    def __init__(self, hidden_size, filter_size, dropout_rate, n_layers):
        ...
        self.last_norms = nn.LayerNorms(hidden_size, eps=1e-6)
...
class Transformer(nn.Module):
    def __init__(self, i_vocab_size, t_vocab_size, n_layers=6, hidden_size=50, filter_size = 50,
                 dropout_rate=0.1, share_target_embedding=True, has_inputs=True, src_pad_idx = None, trg_pad_idx = None):
    ...
    def decode(self, targets, enc_output, i_mask, t_self_mask, t_mask, cache=None):
        ...
        #lets decode
        decode_output = self.decoder(target_embedded, enc_output, i_mask, t_self_mask, cache)
    ...
    def get_postion_encoding(self, x):
        max_length = x.size()[1]
